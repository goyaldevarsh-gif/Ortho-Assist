...
function buildSearchIndex() {
  ...
  papersLibrary.forEach((p) => {
    idx.push({ label: p.title, moduleLabel: "My Papers \u2014 " + p.category, screenId: "papers", paperId: p.id });
  });
  return idx;
}
...
function goToSearchResult(item) {
  ...
  if (item.paperId) { expandedPaperId = item.paperId; papersFormMode = null; }
...
// ---------- PAPERS LIBRARY ----------
const defaultPaperCategories = ["Shoulder", "Spine", "Knee", "Hip", "Elbow", "Wrist & Hand", "Foot & Ankle", "Trauma (general)", "Sports Medicine", "Other"];
let paperCategories = [];
try {
  paperCategories = JSON.parse(localStorage.getItem("orthoAssistPaperCategories") || "null") || [...defaultPaperCategories];
} catch (e) {
  paperCategories = [...defaultPaperCategories];
}
function savePaperCategories() {
  localStorage.setItem("orthoAssistPaperCategories", JSON.stringify(paperCategories));
}

// IndexedDB for storing uploaded PDF blobs (too large for localStorage)
function openPapersDB() {
  return new Promise((resolve, reject) => {
    const req = indexedDB.open("orthoAssistFilesDB", 1);
    req.onupgradeneeded = () => {
      req.result.createObjectStore("paperFiles", { keyPath: "id" });
    };
    req.onsuccess = () => resolve(req.result);
    req.onerror = () => reject(req.error);
  });
}
async function savePaperFile(id, file, name) {
  const db = await openPapersDB();
  return new Promise((resolve, reject) => {
    const tx = db.transaction("paperFiles", "readwrite");
    tx.objectStore("paperFiles").put({ id, blob: file, name: name || file.name || "document.pdf" });
...
async function getPaperFile(id) {
  const db = await openPapersDB();
  return new Promise((resolve, reject) => {
    const tx = db.transaction("paperFiles", "readonly");
    const req = tx.objectStore("paperFiles").get(id);
    req.onsuccess = () => resolve(req.result || null);
    req.onerror = () => reject(req.error);
  });
}
async function deletePaperFile(id) {
  const db = await openPapersDB();
  return new Promise((resolve, reject) => {
    const tx = db.transaction("paperFiles", "readwrite");
    tx.objectStore("paperFiles").delete(id);
    tx.oncomplete = () => resolve();
    tx.onerror = () => reject(tx.error);
  });
}
...
let papersLibrary = [];
try {
  papersLibrary = JSON.parse(localStorage.getItem("orthoAssistPapers") || "[]");
} catch (e) {
  papersLibrary = [];
}
function savePapersLibrary() {
  localStorage.setItem("orthoAssistPapers", JSON.stringify(papersLibrary));
}

let papersFilter = "All";
let papersFormMode = null; // null | "add" | paperId being edited | "categories"
let expandedPaperId = null;
let papersSearchQuery = "";
let papersSortMode = "newest"; // newest | oldest | az

function renderPapers() {
  screenTitle.textContent = "My Papers";

  if (papersFormMode === "categories") {
    renderCategoryManager();
    return;
  }
  if (papersFormMode !== null) {
    renderPaperForm();
    return;
  }

  const counts = {};
  papersLibrary.forEach((p) => { counts[p.category] = (counts[p.category] || 0) + 1; });

  let html = `
    <button class="mgmtToggle" id="addPaperBtn" style="background:var(--teal);border-color:var(--teal);color:#fff;">+ Add a paper</button>
    <div style="display:flex;gap:10px;margin-bottom:16px;">
      <button class="resetLink" id="exportPapersBtn" style="flex:1;text-align:center;border:1px solid var(--border);border-radius:var(--radius-sm);padding:9px;text-decoration:none;">Export library</button>
      <label class="resetLink" id="importPapersLabel" style="flex:1;text-align:center;border:1px solid var(--border);border-radius:var(--radius-sm);padding:9px;text-decoration:none;cursor:pointer;">Import library<input type="file" id="importPapersInput" accept="application/json" style="display:none;"></label>
    </div>
    <div class="badgeRow" id="papersFilterRow">
      <button class="miniBadge ${papersFilter === "All" ? "active" : ""}" data-filter="All" style="${papersFilter === "All" ? "background:var(--teal);color:#fff;" : ""}">All (${papersLibrary.length})</button>`;
  paperCategories.forEach((c) => {
    if (!counts[c]) return;
    html += `<button class="miniBadge" data-filter="${c}" style="${papersFilter === c ? "background:var(--teal);color:#fff;" : ""}">${c} (${counts[c]})</button>`;
  });
  html += `<button class="miniBadge" id="manageCategoriesBtn" style="border:1px dashed var(--border-strong);">Edit categories</button></div>`;

  html += `
    <div class="searchBar" style="margin-bottom:10px;">
      ${ICONS.searchSmall}
      <input type="text" id="papersSearchInput" placeholder="Search your papers\u2026" value="${papersSearchQuery.replace(/"/g, "&quot;")}" />
    </div>
    <div class="badgeRow" style="margin-bottom:16px;">
      <span style="font-size:11px;color:var(--text-3);font-weight:700;align-self:center;margin-right:2px;">SORT</span>
      <button class="miniBadge" data-sort="newest" style="${papersSortMode === "newest" ? "background:var(--teal);color:#fff;" : ""}">Newest</button>
      <button class="miniBadge" data-sort="oldest" style="${papersSortMode === "oldest" ? "background:var(--teal);color:#fff;" : ""}">Oldest</button>
      <button class="miniBadge" data-sort="az" style="${papersSortMode === "az" ? "background:var(--teal);color:#fff;" : ""}">A\u2013Z</button>
    </div>
    <div id="papersListBox"></div>
    <div class="disclaimer">Saved in this browser only \u2014 not synced across devices, and separate from patient data (which is never stored). Export includes attached PDFs (encoded into the file), so it's a full backup \u2014 large libraries with many PDFs may take a few seconds to prepare and produce a bigger file. AI-assisted summarisation is planned for a future update.</div>`;

  app.innerHTML = html;
  refreshPapersList();

  document.getElementById("papersSearchInput").addEventListener("input", (e) => {
    papersSearchQuery = e.target.value;
    refreshPapersList();
  });
  app.querySelectorAll("[data-sort]").forEach((btn) => {
    btn.addEventListener("click", () => {
      papersSortMode = btn.dataset.sort;
      renderPapers();
    });
  });
  ...
  document.getElementById("exportPapersBtn").addEventListener("click", async () => {
    const btn = document.getElementById("exportPapersBtn");
    const originalLabel = btn.textContent;
    btn.textContent = "Preparing\u2026";
    const exportData = [];
    for (const p of papersLibrary) {
      const entry = { ...p };
      if (p.hasFile) {
        try {
          const rec = await getPaperFile(p.id);
          if (rec && rec.blob) {
            entry.fileData = await blobToBase64(rec.blob);
            entry.fileName = rec.name;
          }
        } catch (err) {
          // skip attaching this file if it can't be read, rest of entry still exports
        }
      }
    ...
    a.href = url;
    a.download = "ortho-assist-papers-" + new Date().toISOString().slice(0, 10) + ".json";
  ...
  document.getElementById("importPapersInput").addEventListener("change", (e) => {
    ...
    reader.onload = async () => {
      try {
        ...
        const existingIds = new Set(papersLibrary.map((p) => p.id));
        let added = 0;
        let filesRestored = 0;
        for (const p of imported) {
          if (!p.title) continue;
          let id = p.id && !existingIds.has(p.id) ? p.id : "p" + Date.now() + Math.random().toString(36).slice(2, 7);
          existingIds.add(id);
          const entry = {
            id,
            title: p.title,
            citation: p.citation || "",
            category: paperCategories.includes(p.category) ? p.category : "Other",
            notes: p.notes || "",
            link: p.link || "",
          };
          if (p.fileData) {
            try {
              const blob = await fetch(p.fileData).then((r) => r.blob());
              await savePaperFile(id, blob, p.fileName || "document.pdf");
              entry.hasFile = true;
              entry.fileName = p.fileName || "document.pdf";
              filesRestored++;
            } catch (err) {
              // PDF restore failed for this entry; metadata still imports
            }
          }
          papersLibrary.push(entry);
          added++;
        }
        savePapersLibrary();
        renderPapers();
        alert(`Imported ${added} paper${added === 1 ? "" : "s"}${filesRestored ? `, including ${filesRestored} attached PDF${filesRestored === 1 ? "" : "s"}` : ""}.`);
      } catch (err) {
  ...
  app.querySelectorAll("[data-filter]").forEach((btn) => {
    btn.addEventListener("click", () => {
      papersFilter = btn.dataset.filter;
      renderPapers();
    });
  });
...
function refreshPapersList() {
  const box = document.getElementById("papersListBox");
  if (!box) return;

  let filtered = papersFilter === "All" ? papersLibrary.slice() : papersLibrary.filter((p) => p.category === papersFilter);
  const q = papersSearchQuery.trim().toLowerCase();
  if (q) {
  ...
  if (filtered.length === 0) {
    html = `<div class="emptyHint">${papersLibrary.length === 0 ? "No papers saved yet \u2014 add the first one above." : "No matches."}</div>`;
  } else {
    filtered.forEach((p) => {
      const open = expandedPaperId === p.id;
      const tags = p.tags && p.tags.length ? p.tags.map((t) => `<span class="miniBadge" style="margin-right:4px;">${t}</span>`).join("") : "";
      html += `
        <div class="testRow" style="align-items:flex-start;padding-top:12px;padding-bottom:12px;cursor:pointer;" data-paperid="${p.id}">
          <button class="resBtn ${p.starred ? "pos active" : "neg"}" data-starpaper="${p.id}" aria-label="Star" style="flex-shrink:0;">${p.starred ? ICONS.starFilled : ICONS.starOutline}</button>
          <span style="display:flex;flex-direction:column;flex:1;min-width:0;">
            ${p.title}
            <span class="sub" style="margin-top:2px;">${p.category}${p.citation ? " \u00b7 " + p.citation : ""}</span>
            ${tags ? `<span style="margin-top:5px;">${tags}</span>` : ""}
          </span>
          <span class="chev">${open ? "\u2212" : "+"}</span>
        </div>`;
      if (open) {
        html += `<div class="refBox">
          <div class="refItem"><div class="refItemBody">${p.notes ? p.notes.replace(/\n/g, "<br>") : "<i>No notes added.</i>"}</div></div>
          ${p.link ? `<div class="refItem"><div class="refItemBody"><b>Link/DOI:</b> ${p.link}</div></div>` : ""}
          ${p.hasFile ? `<div class="refItem"><div class="refItemBody"><b>Attached:</b> ${p.fileName || "PDF"} &nbsp; <button class="resetLink" data-openpdf="${p.id}">Open</button></div></div>` : ""}
          <div class="refItem" style="display:flex;gap:10px;">
            <button class="resetLink" data-copycite="${p.id}">Copy citation</button>
            <button class="resetLink" data-editpaper="${p.id}">Edit</button>
            <button class="resetLink" data-deletepaper="${p.id}" style="color:var(--red-text);">Delete</button>
          </div>
        </div>`;
      }
    });
  ...
  box.querySelectorAll("[data-paperid]").forEach((row) => {
  ...
  box.querySelectorAll("[data-editpaper]").forEach((btn) => {
    btn.addEventListener("click", (e) => {
      e.stopPropagation();
      papersFormMode = btn.dataset.editpaper;
      renderPapers();
    });
  });
  box.querySelectorAll("[data-deletepaper]").forEach((btn) => {
    btn.addEventListener("click", async (e) => {
      e.stopPropagation();
      const id = btn.dataset.deletepaper;
      papersLibrary = papersLibrary.filter((p) => p.id !== id);
      savePapersLibrary();
      try { await deletePaperFile(id); } catch (err) {}
      renderPapers();
    });
  });
  box.querySelectorAll("[data-openpdf]").forEach((btn) => {
    btn.addEventListener("click", async (e) => {
      e.stopPropagation();
      const rec = await getPaperFile(btn.dataset.openpdf);
      if (rec && rec.blob) {
  ...
  box.querySelectorAll("[data-starpaper]").forEach((btn) => {
    btn.addEventListener("click", (e) => {
      e.stopPropagation();
      const p = papersLibrary.find((x) => x.id === btn.dataset.starpaper);
      if (p) {
        p.starred = !p.starred;
        savePapersLibrary();
        refreshPapersList();
  ...
  box.querySelectorAll("[data-copycite]").forEach((btn) => {
    btn.addEventListener("click", (e) => {
      e.stopPropagation();
      const p = papersLibrary.find((x) => x.id === btn.dataset.copycite);
      if (!p) return;
      const text = p.citation ? `${p.title} \u2014 ${p.citation}` : p.title;
...
function renderCategoryManager() {
  screenTitle.textContent = "Edit categories";
  const counts = {};
  papersLibrary.forEach((p) => { counts[p.category] = (counts[p.category] || 0) + 1; });

  let html = `<p style="font-size:13px;color:var(--text-2);margin-bottom:14px;">Rename a category by editing its text, or remove one \u2014 papers in a removed category move to \u201cOther.\u201d</p>`;
  paperCategories.forEach((c, i) => {
    const isOther = c === "Other";
    html += `
      <div style="display:flex;gap:8px;align-items:center;margin-bottom:8px;">
        <input type="text" class="formInput" data-catindex="${i}" value="${c.replace(/"/g, "&quot;")}" style="margin-bottom:0;flex:1;" ${isOther ? "disabled" : ""} />
        <button class="resBtn neg" data-delcat="${i}" aria-label="Delete category" ${isOther ? "disabled style=\"opacity:.3;\"" : ""}>${ICONS.x}</button>
      </div>`;
  });
  html += `
    <div class="sectionLabel">Add a category</div>
    <div style="display:flex;gap:8px;">
      <input type="text" id="newCatInput" class="formInput" placeholder="e.g. Pediatric Ortho, Biomechanics" style="margin-bottom:0;flex:1;" />
      <button class="resBtn pos active" id="addCatBtn" aria-label="Add category">${ICONS.check}</button>
    </div>
    <div style="display:flex;gap:10px;margin-top:20px;">
      <button class="mgmtToggle" id="catDone" style="background:var(--teal);border-color:var(--teal);color:#fff;flex:1;margin:0;">Done</button>
    </div>`;

  app.innerHTML = html;

  app.querySelectorAll("[data-catindex]").forEach((input) => {
    input.addEventListener("change", () => {
      const i = parseInt(input.dataset.catindex, 10);
      const oldName = paperCategories[i];
      const newName = input.value.trim();
      if (!newName || newName === oldName) { input.value = oldName; return; }
      if (paperCategories.includes(newName)) {
        alert("That category already exists.");
        input.value = oldName;
        return;
      }
      paperCategories[i] = newName;
      papersLibrary.forEach((p) => { if (p.category === oldName) p.category = newName; });
      savePaperCategories();
      savePapersLibrary();
      renderCategoryManager();
    });
  });
  app.querySelectorAll("[data-delcat]").forEach((btn) => {
    btn.addEventListener("click", () => {
      const i = parseInt(btn.dataset.delcat, 10);
      const name = paperCategories[i];
      const affected = counts[name] || 0;
      if (affected > 0 && !confirm(`${affected} paper${affected === 1 ? "" : "s"} in "${name}" will move to "Other". Continue?`)) return;
      papersLibrary.forEach((p) => { if (p.category === name) p.category = "Other"; });
      paperCategories.splice(i, 1);
      savePaperCategories();
      savePapersLibrary();
      renderCategoryManager();
    });
  ...
  document.getElementById("addCatBtn").addEventListener("click", () => {
    ...
    if (!name) return;
    if (paperCategories.includes(name)) {
      alert("That category already exists.");
      return;
    }
    paperCategories.splice(paperCategories.length - 1, 0, name); // insert before "Other"
    savePaperCategories();
    renderCategoryManager();
  });
...
function renderPaperForm() {
  const editing = papersFormMode !== "add" ? papersLibrary.find((p) => p.id === papersFormMode) : null;
  screenTitle.textContent = editing ? "Edit paper" : "Add a paper";

  let html = `
    <div class="sectionLabel">Title</div>
    <input type="text" id="pfTitle" class="formInput" placeholder="Paper title" value="${editing ? editing.title.replace(/"/g, "&quot;") : ""}" />
    <div class="sectionLabel">Citation</div>
    <input type="text" id="pfCitation" class="formInput" placeholder="Authors, journal, year" value="${editing ? editing.citation.replace(/"/g, "&quot;") : ""}" />
    <div class="sectionLabel">Category</div>
    <div class="selectGrid" id="pfCategoryGrid">`;
  paperCategories.forEach((c) => {
    const sel = (editing ? editing.category : "Shoulder") === c;
    html += `<button class="optRow ${sel ? "selected" : ""}" data-catopt="${c}"><span>${c}</span><span class="check">${sel ? ICONS.check : ""}</span></button>`;
  });
  html += `</div>
    <div class="sectionLabel">Key takeaway / notes</div>
    <textarea id="pfNotes" class="formInput formTextarea" placeholder="What's useful about this paper \u2014 findings, why it changed your practice, anything worth remembering">${editing ? editing.notes || "" : ""}</textarea>
    <div class="sectionLabel">Tags (optional)</div>
    <input type="text" id="pfTags" class="formInput" placeholder="e.g. cadaveric study, RCT, biomechanics \u2014 comma separated" value="${editing && editing.tags ? editing.tags.join(", ") : ""}" />
    <div class="sectionLabel">Link / DOI (optional)</div>
    <input type="text" id="pfLink" class="formInput" placeholder="https:\u2044\u2044..." value="${editing && editing.link ? editing.link.replace(/"/g, "&quot;") : ""}" />
    <div class="sectionLabel">Attach PDF (optional)</div>
    <input type="file" id="pfFile" class="formInput" accept="application/pdf" />
    ${editing && editing.hasFile ? `<p style="font-size:12px;color:var(--text-3);margin:-8px 0 14px;">Currently attached: ${editing.fileName || "PDF"}. Choosing a new file replaces it.</p>` : ""}
    <div style="display:flex;gap:10px;margin-top:18px;">
      <button class="mgmtToggle" id="pfSave" style="background:var(--teal);border-color:var(--teal);color:#fff;flex:1;margin:0;">Save</button>
      <button class="mgmtToggle" id="pfCancel" style="flex:1;margin:0;">Cancel</button>
    </div>`;

  app.innerHTML = html;
  let selectedCategory = editing ? editing.category : "Shoulder";
  document.querySelectorAll("[data-catopt]").forEach((btn) => {
    btn.addEventListener("click", () => {
      selectedCategory = btn.dataset.catopt;
      document.querySelectorAll("[data-catopt]").forEach((b) => {
        const isSel = b.dataset.catopt === selectedCategory;
        b.classList.toggle("selected", isSel);
        b.querySelector(".check").innerHTML = isSel ? ICONS.check : "";
      });
    });
  });
  ...
  document.getElementById("pfSave").addEventListener("click", async () => {
    ...
    const tagsRaw = document.getElementById("pfTags").value.trim();
    const data = {
      title,
      citation: document.getElementById("pfCitation").value.trim(),
    ...
    if (editing) {
      ...
      data.starred = false;
      papersLibrary.push(data);
    }
    if (chosenFile) {
      try {
        await savePaperFile(targetId, chosenFile, chosenFile.name);
        const rec = papersLibrary.find((p) => p.id === targetId);
        rec.hasFile = true;
        rec.fileName = chosenFile.name;
      } catch (err) {
        alert("Couldn't save the PDF \u2014 it may be too large for this device's storage. The rest of the entry was saved.");
      }
    }
    savePapersLibrary();
    papersFormMode = null;
...
function render() {
  ...
  else if (currentScreen === "papers") renderPapers();
...
function renderAppHome() {
  ...
  html += `</div>
    ...
    <button class="homeCard c-amber" id="goPapers">
      <div class="iconBadge">${ICONS.book}</div>
      <div style="flex:1;">
        <div class="title">My Papers</div>
        <div class="desc">Your own library \u2014 add papers, organised by joint/specialty</div>
      </div>
      <div class="chev">${ICONS.chevRight}</div>
    </button>`;
  app.innerHTML = html;
  ...
  document.getElementById("goPapers").addEventListener("click", () => { papersFormMode = null; setScreen("papers"); });
